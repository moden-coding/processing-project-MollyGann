...
public class App extends PApplet{
    ...
    float moveSpeed = 2; // Speed of movement to the left
    float resetPosition = width; // Reset position to the right side of the screen

    float circleY = 368; // Initial Y position of the circle
    ...
    public void draw(){
        ...
        if (isJumping) {
            circleY -= 3; // Move up
            if (circleY < 368 - jumpHeight) { // Check if reached jump height
                isJumping = false; // Start falling
            }
        } else {
            if (circleY < 368) { // If falling, reset to ground level
                circleY += 3; // Move down
            }
        }
    stroke(235, 130, 49); 
        ...
        if (triangleX1 < -10) triangleX1 = width;
        if (triangleX2 < -10) triangleX2 = width;
        if (triangleX3 < -10) triangleX3 = width;
        if (triangleX4 < -10) triangleX4 = width;
        if (triangleX5 < -10) triangleX5 = width;
